...
N_SUBJECTS=$(( $( wc -l ${BIDS_DIR}/participants_sessions_tl.tsv | cut -f1 -d' ' ) - 1 ))
array_job_length=$(echo "$N_SUBJECTS/${SUB_SIZE}" | bc)
Tail=$((N_SUBJECTS-(array_job_length*SUB_SIZE)))

if [ "$SLURM_ARRAY_TASK_ID" -eq "$array_job_length" ]; then
    SUBJECTS=`sed -n -E "s/sub-(\S*)\>.*/\1/gp" ${BIDS_DIR}/participants_sessions_tl.tsv  | head -n ${N_SUBJECTS} | tail -n ${Tail}`
else
    SUBJECTS=`sed -n -E "s/sub-(\S*)\>.*/\1/gp" ${BIDS_DIR}/participants_sessions_tl.tsv | head -n ${bigger_bit} | tail -n ${SUB_SIZE}`
fi
...
#     find ${LOCAL_FREESURFER_DIR}/sub-$subject.long.*/ -name "*IsRunning*" -type f -delete
# done

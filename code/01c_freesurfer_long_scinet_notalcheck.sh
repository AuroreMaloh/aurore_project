...
N_SUBJECTS=$(( $( wc -l ${BIDS_DIR}/participants_sessions.tsv | cut -f1 -d' ' ) - 1 ))
array_job_length=$(echo "$N_SUBJECTS/${SUB_SIZE}" | bc)
Tail=$((N_SUBJECTS-(array_job_length*SUB_SIZE)))

if [ "$SLURM_ARRAY_TASK_ID" -eq "$array_job_length" ]; then
    sub_ses=`cat ${BIDS_DIR}/participants_sessions.tsv  | head -n ${N_SUBJECTS} | tail -n ${Tail}`
else
    sub_ses=`cat ${BIDS_DIR}/participants_sessions.tsv | head -n ${bigger_bit} | tail -n ${SUB_SIZE}`
fi

## need variable for subject = $SUBJECT
## need variable for sessions = $SESSION
SUBJECT=`echo ${sub_ses} | cut -d "_" -f 1`
SESSION=`echo ${sub_ses} | cut -d "_" -f 2`
...
rm -rf ${OUTPUT_DIR}/${SUBJECT}_${SESSION}/
...
singularity exec --cleanenv \
    ...
    ${SING_CONTAINER} \
    recon-all -subjid ${SUBJECT}_${SESSION} \
    -sd /derived \
    -3T \
    -i /bids/${SUBJECT}/${SESSION}/anat/${SUBJECT}_${SESSION}_T1w.nii.gz \
    -autorecon-all \
    -openmp 80 \
    -notal-check
